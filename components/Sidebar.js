...
export default function Sidebar({ user: propUser }) {
  ...
  const navItems = [
    ...
    { label: 'Profile', href: user ? `/profile?username=${user.user_metadata?.username || 'user'}` : '/login', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> },
  ];
